...
process_persistent_datalog_assertion(persistent(PredSchema,Connection),_Ls,_FId,_Action,_Error) :-
  ...
  (var(Error)
   ...
   ;
    process_processing_error(['Trying to make ',PredSchema,' persistent'])
  ).
...
check_existing_pred_schema(PredSchema) :-
  ...
  ( get_table_types('$des',Name,DTypes),
    ...
    !
  ;
    set_flag(processing_error([': Existing and incompatible schema found for ',Name])),
    fail
  ).
...
translate_predicate_rule(Connection,DBMS,DLRule,DQL) :-
  ...
  NVRule = (Rule,NVs),
%  dx_simplify_rule(Rule,SRule), % Moved to my_translate/4
  my_translate(Connection,DBMS,(Rule,NVs),DQL).

my_translate_list([],[]).
my_translate_list([RuleNVs|RuleNVss],[DQL|DQLs]) :-
  my_translate(RuleNVs,DQL),
  my_translate_list(RuleNVss,DQLs).

my_translate(RuleNVs,DQL) :-
  current_db(Connection,DBMS),
  my_translate(Connection,DBMS,RuleNVs,DQL).

% If a given Datalog rule cannot be translated to SQL, this predicate warns it and fails
my_translate(Connection,DBMS,(Rule,NVs),DQL) :-
  ...
  ensure_safe_ruleNVs((SRule,NVs)),
  once(translate(Connection,Head,Body,DQL)),
  ground(DQL). % There should be no variables in the translation. They can happen in rules as: v(X):-t(Y),X = Y+1 
               % because equality (=) binds in Datalog, rather than comparing (which would be translated into a comparison in the where clause)
my_translate(_Connection,_DBMS,_RuleNVs,_DQL) :-
%  write_warning_log(['The rule ','$exec'(write_with_NVs(Rule,NVs)),'. cannot be translated to SQL']),
  !,
  fail.
    
/*******************************************************************/
/* Display a Datalog program as a set of equivalent SQL statements */
% Given a set of Datalog rules, display an equivalent SQL statement
% display_equivalent_SQL_to_DL_program(Language,RNVss) :-
%   show_sql(on),
%   once((Language == 'RA' ; Language == 'DRC' ; Language == 'TRC')),
%   dl_program_to_sql_statements(RNVss,SQLs),
%   write_info_log(['Equivalent SQL query optimized to:']),
%   nl_compact_log,
%   display_sql_list(SQLs,0),
%   !.
% display_equivalent_SQL_to_DL_program(_Language,_DRNVss).

/**********************************************************************/
/* Translates a Datalog program to a set of equivalent SQL statements */
dl_program_to_sql_statements(RNVss,SQLs) :-
  ruleNVss_by_predicate(RNVss,PRNVsss), % Delivers a list of lists Li, where all rules in Li belongs to the same predicate
  translate_predicate_to_sql_list(PRNVsss,SQLs),
  !.
  
ruleNVss_by_predicate(RNVss,PRNVsss) :-
  ruleNVss_by_predicate(RNVss,[],PRNVsss).

ruleNVss_by_predicate([],PRNVsss,PRNVsss).
ruleNVss_by_predicate([(R,NVs)|RNVss],PRNVsssi,PRNVssso) :-
  rule_pred(R,P),
  add_rule_to_pred_rules_list((R,NVs),P,PRNVsssi,PRNVsssi1),
  ruleNVss_by_predicate(RNVss,PRNVsssi1,PRNVssso).

add_rule_to_pred_rules_list((R,NVs),P,[(P,RNVssi)|PRNVsss],[(P,RNVsso)|PRNVsss]) :-
  !,
  append(RNVssi,[(R,NVs)],RNVsso).
add_rule_to_pred_rules_list((R,NVs),P,[PRNVss|PRNVsssi],[PRNVss|PRNVssso]) :-
  add_rule_to_pred_rules_list((R,NVs),P,PRNVsssi,PRNVssso),
  !.
add_rule_to_pred_rules_list((R,NVs),P,PRNVsssi,PRNVssso) :-
  append(PRNVsssi,[(P,[(R,NVs)])],PRNVssso).
/*******************************************************************/

translate_predicate_to_sql_list([],[]).
translate_predicate_to_sql_list([PRNVss|PRNVsss],[SQL|SQLs]) :-
  translate_predicate_to_sql(PRNVss,SQL),
  translate_predicate_to_sql_list(PRNVsss,SQLs).
  
translate_predicate_to_sql((_P,RNVss),SQL) :-
  my_translate_list(RNVss,DQLs), % DQLs: Draxler representation
  current_db(Connection,_DBMS),
  compose_unions(Connection,DQLs,StrQuery),
  parse_sql_query(SQL,StrQuery). % SQL: DES representation
...
% --------------------------------------------------------------------------------------
%
% Top level predicate translate/4 organizes the compilation and constructs a
...
% evaluable_expression(_Connection,'$var$'(VarId),Dictionary,att(RangeVar,Attribute),EType):-
%    lookup(VarId,Dictionary,RangeVar,Attribute,VType),
%    RangeVar \= (is),
%    type_compatible(VType,EType). % FSP: Added. E.g. t(Y),X=Y+1, for t(a integer): integer \== number This should be a type compatibility test, instead of type matching
...
% of ASCII codes is created as a difference list, and it is then 
...
my_query_atom(_Connection,Query,_QueryList,_Diff) :-
  var(Query),
  !,
  write_error_log(['Unsafe query: ',Query,'. Cannot be translated to SQL.']),
  fail.
my_query_atom(Connection,Query,QueryList,Diff) :-
...
query_atom(_Connection,Var,_QueryList,_Diff):-
   var(Var),
   !,
   fail.
...
% -- Type conversions:
% FSP: Type conversions to top types, otherwise they are not handled properly (cf. evaluable_expression, where type mismatches occur between numeric types, as trying to bind integer to number)
dx_des_datatype(number,number(float)).
dx_des_datatype(number,number(integer)).
dx_des_datatype(string,string(varchar)).
dx_des_datatype(string,string(varchar(_))).
dx_des_datatype(date,datetime(date)).
dx_des_datatype(time,datetime(time)).
dx_des_datatype(datetime,datetime(datetime)).
dx_des_datatype(timestamp,datetime(datetime)).
...
% FSP:
% dx_get_type('$const$'(Constant),integer):- % This is in the original, changed to:
%    number(Constant).
