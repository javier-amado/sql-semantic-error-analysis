...
applyTuple(Tc,FLt, Ltables1, (exists(select(_,_,_,_La,_TL,
                                        from(Ltables2), 
                                        where(Predicate),
                                        group_by(_), 
                                        having(_H),
...
applyTuple(Tc,FLt, Ltables1, (notExists(select(_,_,_,_La,_TL,
                                    from(Ltables2), 
                                    where(Predicate),
                                    group_by(_), 
                                    having(_H),
...
collectTables(true, []) :- !.
collectTables(false, []) :- !.
collectTables(select(_,_,_,_Latt,_TL,
                    from(M), 
                    where(P), 
                    group_by(_), 
                    having(_H),
...
% EXISTS: exists((SQ, _))
translatePred(ptc, exists((select(_, _, _, L, _, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_)),
                   LTables, 
                   exists(select(_, _, _, L, _, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).
      
translatePred(ntc, exists((select(_, _, _, L, _, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_)),
                   LTables, 
                   notExists(select(_, _, _, L, _, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).
      
% NOT EXISTS : not(exists((SQ, _)))                                              
translatePred(ptc, not(exists((select(_, _, _, L, _, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_))),
                   LTables, 
                   notExists(select(_, _, _, L, _, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).  
      
translatePred(ntc, not(exists((select(_, _, _, L, _, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_))),
                   LTables, 
                   exists(select(_, _, _, L, _, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond). 
...
translatePred(ptc, AnyCond,
           LTables, 
           % :::WARNING: all?
           exists(select(_, _, _, all, _, from(M), where(and(Cond,bc(Ro1, Op, Ro2))), 
           group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2)))) :-
           
      AnyCond=..Res, Res=[Type, E1, (SQ, _ )],  mySqType(Type, Op, any),
      SQ = select(_, _, _, [E2], _, from(M), where(C), group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2)),         
...
translatePred(ptc, AllCond,                        
              LTables, 
            notExists(select(_, _, _, Ro2, _, from(M), where(and(Cond,bc(Ro1,NOp,Ro2) )),
                      group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2) ))) :- 
...
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%get_attrView(select(_, _, _, Exp, _, 
%             from(_Tables), 
%             where(_Pred_w),  
%             group_by(_AttGb),
%             having(_Pred_h), 
...
%             Exp).
%
%get_fromView(select(_, _, _, _Exp, _, 
%             from(Tables), 
%             where(_Pred_w),  
%             group_by(_AttGb),
%             having(_Pred_h), 
...
%             Tables).
%                 
%get_whereView(select(_, _, _, _Exp, _, 
%              from(_Tables), 
%              where(Pred_w),  
%              group_by(_AttGb),
%              having(_Pred_h), 
...
%              Pred_w).
%                 
%get_groupbyView(select(_, _, _, _Exp, _, 
%								from(_Tables), 
%								where(_Pred_w),  
%								group_by(AttGb),
%								having(_Pred_h), 
...
%                AttGb).
%                 
%get_havingView(select(_, _, _, _Exp, _, 
%								from(_Tables), 
%								where(_Pred_w),  
%								group_by(_AttGb),
%								having(Pred_h), 
