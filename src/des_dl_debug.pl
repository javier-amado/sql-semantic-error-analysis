...
% :-use_module(library(chr)). % Already loaded

% Dynamic predicates (others for statistics are shared with SQL debugger)

:- dynamic(debug_dl_buggy/1).             % buggy(BuggySource)
:- dynamic(debug_dl_current_question/1).  % Current debugging question under TAPI
:- dynamic(debug_dl_state/1).             % State of the current debugging session
:- dynamic(debug_dl_current_constraint/1).% Current debugging constraint under TAPI
:- dynamic(debug_dl_options/1).           % Options
:- dynamic(debug_dl_file/1).              % File to debug

% Reset debugging session

reset_DL_debug_session :-
  retractall(debug_dl_buggy(_)),
  retractall(debug_dl_current_question(_)),
  retractall(debug_dl_state(_)),
  retractall(debug_dl_current_constraint(_)),
  retractall(debug_dl_options(_)),
  retract(debug_dl_file(AbsFile)),
  debug_dl_files(AbsFile,OutTransformFile,OutBridgeFile),
  delete_file_if_exists(OutTransformFile),
  delete_file_if_exists(OutBridgeFile),
  !.
reset_DL_debug_session.

% Start of full debugging (including missing and wrong answers)

debug_dl_full(RelationName,Arity,Options) :-
  (member(file(FileName),Options)
   ->
    my_absolute_file_name(FileName,AbsFile),
    exec_if(tapi(on),(output(Output),set_flag(output,off))),
    % consult resets the database (and, thus, any debugging session)
    processC(consult,[AbsFile],[],yes),
    exec_if(tapi(on),set_flag(output,Output)),
    user_predicate(RelationName/Arity),
    exec_if_development_on(
      write_info_log(['Original Datalog program:']),
      cat_file(AbsFile),
      nl_log)
   ;
    % nofile: use the contents of the database
    user_predicate(RelationName/Arity),
    FileName='_debug_dl_default_file.dl',
    my_absolute_filename(FileName,AbsFile),
    write_notapi_info_log(['Saving Datalog program to: ',AbsFile]),
    exec_if_development_on(
      write_info_log(['Original Datalog program:']),
      processC(listing,[],[],yes),
      no_log),
    processC(save_ddb,[force,AbsFile],[],yes)
  ),
  !,
%  set_flag(debug_dl_file,AbsFile),
  debug_dl_files(AbsFile,OutTransformFile,OutBridgeFile),
%   set_flag(debug_questions,0),
%   set_flag(debug_tuples,0),
  write_notapi_info_log(['Loading and transforming file...']),
...
%  catch(my_ensure_loaded(OutBridgeFile), M, (nl,write(M),nl)), 
  ...
  retractall(debug_dl_file(_)), % Remove this flag to avoid deleting transformed programs in consult (due to reset_database, which implies reset_DL_debug_session)
  push_flag(output,off,CurrentOutput),
  processC(consult,[OutTransformFile],[],yes),
  pop_flag(output,CurrentOutput),
  % Set initialization flags after the last consult (it resets the database)
  set_flag(debug_dl_file,AbsFile),
  set_flag(debug_questions,0),
  set_flag(debug_tuples,0),
  debug_dl_start_debug_dl(RelationName,Arity,Options),
  exec_if(tapi(off),debug_dl_restore_ddb(FileName)).
debug_dl_full(RelationName,Arity,_Options) :-
  write_error_log(['Predicate ',RelationName/Arity,' is not a user predicate or not defined.']).
  
  
% Determine the file names for the transformed and the bridge programs from the debugged Datalog file
% debug_dl_files(+AbsFile,-OutTransformFile,-OutBridgeFile) 
debug_dl_files(AbsFile,OutTransformFile,OutBridgeFile) :-
...
debug_dl_restore_ddb :-
  debug_dl_file(FileName),
  debug_dl_restore_ddb(FileName).

debug_dl_restore_ddb(FileName) :-
  write_notapi_info_log(['Restoring original database...']),
  % Save debugger state
  DebugState=[
    debug_dl_buggy(_),
    debug_dl_current_question(_),
    debug_dl_state(_),
    debug_dl_current_constraint(_),
    debug_dl_options(_),
    debug_dl_file(_),
    debug_questions(_),
    debug_tuples(_),
    debug_root_tuples(_)],
  findall(X,(member(X,DebugState),X),Xs), % Some flags might not be set yet
  push_flag(output,off,CurrentOutput),
  processC(consult,[FileName],[],yes),
  pop_flag(output,CurrentOutput),
  % Restore debugger state
  findall(_,(member(X,Xs),set_flag(X)),_). 
...
debug_dl_process_answer(_Question,abort) :-
  !,
  debug_dl_restore_ddb,
  reset_DL_debug_session,
  write_tapi_success.
debug_dl_process_answer(Question,Answer) :-
  (finished_dl_debugging
   ->
    write_error_log(['Nothing left to do. Current debugging session finished.'])
   ;
    (debug_dl_check_answer(Question,Answer)
     ->
      debug_dl_options(Options),
      debug_dl_current_constraint(Constraint),
      debug_dl_process_answer(Options,Constraint,Question,Answer)
     ;
...
finished_dl_debugging :- 
  debug_dl_current_question([]). % WARNING
debug_dl_check_answer(_Question,_Answer). % WARNING

% debug_dl_process_answer(+Options,+Constraint,+Question,+Answer)
debug_dl_process_answer(Options,AccGoal,Question,Answer) :- 
  % Restore the CHR constraint store by re-posting the accumulated constraint
  % TODO: Save and restore the CHR constraint store
  call(AccGoal),
  debug_dl_ask_user(Question,Answer,Options,Constraint,Abort,Error),
  append_goals(AccGoal,Constraint,NewAccGoal),
  % Source must be a variable (this re-post the accumulated constraint)
  debug_dl_loop(Constraint,Options,_Source,BuggyNodes,NewAccGoal,Abort,Error),
  debug_dl_finish_with_buggy(Abort,BuggyNodes).
  
debug_dl_finish_with_buggy(Abort,_BuggyNodes) :-
  nonvar(Abort),
  !,
  debug_dl_restore_ddb.
debug_dl_finish_with_buggy(_Abort,BuggyNodes) :-
  nonvar(BuggyNodes),
  !,
  set_flag(debug_dl_buggy(BuggyNodes)),
  set_flag(debug_dl_current_question([])),
  (tapi(on)
   ->
  ...
  debug_dl_restore_ddb.
debug_dl_finish_with_buggy(_Abort,_BuggyNodes).

display_dl_node_states :-
  debug_dl_rel_state_list(AllRelStateList),
  my_unzip(AllRelStateList,Rels,States),
  merge_zip(Rels,States,RelsStates),
  write_log_lines(RelsStates),
  write_tapi_eot.
  
debug_dl_rel_state_list(AllRelStateList) :-
  debug_dl_state(Ctr1),
  debug_dl_current_constraint(Ctr2),
  my_list_to_tuple(Ctrs1,Ctr1),
  my_list_to_tuple(Ctrs2,Ctr2),
  my_set_union(Ctrs1,Ctrs2,UCtrs),
  (buggy_relations(BuggyRelations) -> true ; BuggyRelations=[]),
  length(BuggyRelations,L),
  length(Errs,L),
  my_map_1('='(erroneous),Errs),
  my_zipWith(',',BuggyRelations,Errs,ErrRelStateList),
  findall((Rel,State),(member(Ctr,UCtrs),ctr_rel_state(Ctr,Rel,State),\+member((Rel,_),ErrRelStateList)),RelStateList),
  my_set_union(ErrRelStateList,RelStateList,AllRelStateList).
  
ctr_rel_state(u_valid(Rel,Validity),Rel,UserValidity) :-
  debug_dl_command_answer_internal_answer(UserValidity,Validity).
  

debug_dl_check_valid_node(Name/_Arity) :-
  clause(chr_mod:relCts,Body),
  !,
  my_list_to_tuple(Goals,Body),
  memberchk(relation(Name,_),Goals).
  
debug_dl_check_valid_state_change(valid).
debug_dl_check_valid_state_change(nonvalid).

debug_dl_set_node(Name/Arity,State) :-
  debug_dl_rel_state_list(AllRelStateList),
  (memberchk((Name,_),AllRelStateList)
   ->
    write_error_log(['Cannot change an already set node.'])
   ;
%     pred_arity(Name,A),
%     !,
    debug_dl_process_answer(eq(c(Name/Arity),i(Name)),u_valid(Name,State))).
...
debug_dl_start_debug_dl(R,N,Options) :-
  Question = eq(c(R/N),i(R)),
  set_flag(debug_dl_current_question(Question)),
  % Check whether there is a know answer for the first question
  (member(answer(Answer),Options)
   ->
    question_answer_constraint(Question,[],Answer,DefaultConstraint),
    set_flag(debug_dl_current_constraint(DefaultConstraint)),
    inc_flag(debug_questions) % Implied (first) question
   ;
    true),
  debug_dl_ask_user(Question,Answer,[root(R/N)|Options],Constraint,Abort,_Error),
%   debug_dl_get_and_display_meaning(RelationName,Arity,Solutions),
%   debug_dl_input_ask_user(RelationName,Arity,Solutions,Constraint,Abort,_Error), 
  (nonvar(Abort)
   ->
    write_info_log(['Debugging aborted by user.']),
    reset_SQL_debug_session
   ;
    debug_dl_extension_goal(Options,ExtGoal),
    % Answer is a goal:
    append_goals_list([cts,Constraint,ExtGoal],Goal),
    % '_Source' is not relevant here because the accumulated constraint is the same as the current constraint (Goal)
    debug_dl_loop(Goal,Options,_Source,BuggyNodes,Goal,Abort,Error),
    (nonvar(Abort)
     ->
     ...
     ;
      (nonvar(Error)
       ...
       ;
        debug_dl_finish_with_buggy(Abort,BuggyNodes),
        exec_if(tapi(off),display_debug_statistics(datalog))
      )
    )
  ).
...
buggy_relations(BuggyRelations) :-
  debug_dl_buggy(BuggyNodes),
  buggy_relations(BuggyNodes,BuggyRelations).

buggy_relations(BuggyNodes,BuggyViewNames) :-
  buggy_relations_aux(BuggyNodes,BuggyViewNames).
  
buggy_relations_aux([],[]).
buggy_relations_aux([buggy([BuggyViewName],buggy,_)|BuggyNodes],[BuggyViewName|BuggyViewNames]) :-
  !,
  buggy_relations_aux(BuggyNodes,BuggyViewNames).
buggy_relations_aux([buggy(BuggyViewName,_,_)|BuggyNodes],[BuggyViewName|BuggyViewNames]) :-
  buggy_relations_aux(BuggyNodes,BuggyViewNames).
...
% Code 1
%
%debug_dl_loop(+Goal,+Options,+Source,-BuggyNodes,+AccGoal,?Abort,?Error) :-
debug_dl_loop(Goal,Options,Source,BuggyNodes,AccGoal,Abort,Error) :-
  write_info_dev_log(['Current goal: ', AccGoal]),
  % Post CHR constraint:
  call(Goal),
  getBuggy(CurrentBuggyNodes),
  (CurrentBuggyNodes == []
   ->
%    call(getQuestions(Questions)),
    (getQuestions(Questions),
     ...
     -> 
      ...
      debug_dl_choose_question_with_constraint(Questions,Source,Options,Constraint,Abort,Error),
      (var(Abort),
       var(Error)
       ->
        write_info_dev_log(['Answer constraint: ',Constraint]),
        append_goals(AccGoal,Constraint,NewAccGoal),
%        ((tapi(off) ; (Source==system, exec_if(tapi(on),NSource=Source)))
        ((tapi(off) ; Source==system)
         ->
          debug_dl_loop(Constraint,Options,_NSource,BuggyNodes,NewAccGoal,Abort,Error)
         ;
          set_flag(debug_dl_current_constraint(NewAccGoal)),
          set_flag(debug_dl_state(Constraint)),
          display_dl_node_states
        )
       ;
        true
     ...
     ;
      set_flag(debug_dl_current_question([])),
      set_flag(debug_dl_current_constraint(AccGoal)),
      exec_if(tapi(on),write_error_log(['Unable to locate a buggy node.'])),
      Error=true % No buggy nodes and no possible questions: cannot locate buggy node or circuit
    )
   ;
    BuggyNodes = CurrentBuggyNodes
  ).
  
debug_dl_extension_goal(Options,Goal) :-
  memberchk(trust_extension(yes),Options),
  extensional_predicates(Ps),
  Ps\==[],
  !,
  % Mark user extensional predicates as valid (not the new ones resulting from the transformation - those starting by '_')
  findall(u_valid(N,yes),(member(N/_A,Ps),\+atom_concat('_',_,N)),Goals),
  append_goals_list(Goals,Goal).
debug_dl_extension_goal(_Options,true).
  
% CHR Rules to be added as a result of a question
% Select first the one with an automated answer (lines 5 and 6 in table 4)

% debug_dl_choose_question_with_constraint(+Questions,-Source(system or user),+Options,-Constraint,-Abort,-Error)
% If the system can not choose a question, it let the user to choose one
debug_dl_choose_question_with_constraint(Questions,system,_Options,Constraint,Abort,Error) :- 
...
debug_dl_choose_question_with_constraint(Questions,_SystemUser,Options,Constraint,Abort,Error) :- 
  debug_dl_choose_user_question_with_constraint(Questions,_Question,Options,Constraint,Abort,Error).
%   ,
...
%   !,
  ...
  append(Vs,RVs,CVs),
  DLQuery =.. [Q|CVs].
% build_DL_query(Q,_Vs,_DLQuery) :-
%   my_raise_exception(unknown_user_predicate(Q/'?'),syntax(''),[]).
...
debug_dl_choose_user_question_with_constraint(Questions,Question,Options,Constraint,Abort,Error) :-
...
%  my_quicksort_pred(UQsCs,second_tuple_arg_desc_order,[(Question,_)|_]),
  ...
  debug_dl_ask_user(Question,_,Options,Constraint,Abort,Error).
...
% debug_dl_ask_user(+Question,+Answer,+Options,-Constraint,-Abort,-Error)
% Ask oracle
% Answer may be bound (an already known answer; so, do not ask user)
debug_dl_ask_user(Question,Answer,Options,Constraint,Abort,Error) :-
  set_flag(debug_dl_options(Options)),
  ...
  (var(Answer)
   ->
    question_possible_user_answers(Question,PossibleAnswers,DefaultAnswer),
...
%    question_answer_constraint(Question,Solutions,Answer,Constraint)
    debug_dl_input_ask_user(Question,UserQuestion,R,N,Solutions,PossibleAnswers,DefaultAnswer,Constraint,Abort,Error)
   ;
    % Implied question: add tuples
    length(Solutions,NbrTuples),
...
question_predicate(eq(c(R/N),_),R/N).
...
question_userquestion(Question,_) :- 
  my_raise_exception(generic,syntax(['Unsupported question for asking user: ',Question]),[]).

debug_dl_question_command_question(eq(c(R/N),i(R)),all(R/N)).
debug_dl_question_command_question(eq(i(R,pi(_N,_M,c(_LS,_X))),empty),empty(R/A)) :-
  pred_arity(R,A),
  !.
debug_dl_question_command_question(eq(i(R,pi(_N,_M,c(_LS,_X))),nonempty),nonempty(R/A)) :-
  pred_arity(R,A),
  !.
debug_dl_question_command_question(subset(pos,pi(_N,_M,c(WS,_X)),i(S)),subset(WS/A,S/A)) :-
  pred_arity(S,A),
  !.
debug_dl_question_command_question(subset(neg,pi(_N,_M,c(WS,_X)),i(S)),nonsubset(WS/A,S/A)) :-
  pred_arity(S,A),
  !.
debug_dl_question_command_question([],[]). % End of debugging
...
%question_answer_constraint(subset(Sign,pi(N,M,c((WS,X))),i(S)),Solutions,Answer,(subset(Sign,pi(N,M,c((WS,X))),i(S),no),u_valid(S,no),Answer)) :-
question_answer_constraint(subset(Sign,pi(N,M,c(WS,X)),i(S)),_Solutions,Answer,(subset(Sign,pi(N,M,c(WS,X)),i(S),no),u_valid(S,no),Answer)) :-
  !. % Answer is either u_missing(t,R) or u_wrong(t,R), so that subset(Sign,pi(N,M,c((WS,X))),i(S),no) is also added
question_answer_constraint(eq(i(S,pi(N,M,(c(LS,X)))),Empty),_Solutions,Answer,eq(i(S,pi(N,M,(c(LS,X)))),Empty,Answer)) :-
  (Answer = yes ; Answer = no),
  !.
question_answer_constraint(Question,_Solutions,Answer,Answer) :-
%  write_error_log(['Unsupported Question: ',Question]).
   my_raise_exception(generic,syntax(['Unsupported question: ',Question]),[]).
    
% For propositional predicates, only yes or no can be answered
question_possible_user_answers(eq(c(R/0),i(R)),
                              [yes,no],
                              no) :- !.
question_possible_user_answers(eq(c(R/_N),i(R)),
                              [yes,no,u_missing(_,R),u_wrong(_,R)],
                              no) :- !.
%question_possible_user_answers(eq(c((_Q,_X)),empty),
question_possible_user_answers(eq(c(_Q,_X),empty),
                              [yes,no],
                              no) :- !.
question_possible_user_answers(in(_X,c(_Ri)),
                              [yes,no],
                              no) :- !.
%question_possible_user_answers(subset(_Sign,pi(_N,_M,c((_WS,_X))),i(S)),
question_possible_user_answers(subset(_Sign,pi(_N,_M,c(_WS,_X)),i(S)),
                              [yes,no,u_missing(_,S),u_wrong(_,S)],
                              no) :- !.
question_possible_user_answers(eq(i(_S,pi(_N,_M,(c(_LS,_X)))),_Empty),
                              [yes,no],
                              no) :- !.
question_possible_user_answers(Question,_PossibleAnswers,_DefaultAnswer) :-
%  write_error_log(['Unsupported Question: ',Question]).
  my_raise_exception(generic,syntax(['Unsupported question: ',Question]),[]).

debug_dl_build_user_question(UserQuestion,_Solutions,PossibleAnswers,DefaultAnswer,CompleteUserQuestion) :-
...
debug_dl_answer_text_user_answers(u_missing(_,_),"mT","\n mT (missing tuple T as a comma-separated list of Datalog constants and placeholders '_')").
debug_dl_answer_text_user_answers(u_wrong(_,_),"wN","\n wN (wrong tuple at position N)").
  
% debug_dl_input_ask_user(+Question,+UserQuestion,+RelationName,+Arity,+Solutions,+PossibleAnswers,+DefaultAnswer,-Constraint,-Abort,-Error)
...
debug_dl_input_ask_user(Question,_UserQuestion,_RelationName,_Arity,Solutions,_PossibleAnswers,_DefaultAnswer,Constraint,_Abort,_Error) :-
  % No solutions for subset
  Solutions=[],
  Question=subset(_Sign,pi(_N,_M,c(_WS,_X)),i(_S)),
  !,
  question_answer_constraint(Question,Solutions,yes,Constraint).
debug_dl_input_ask_user(Question,UserQuestion,RelationName,Arity,Solutions,PossibleAnswers,DefaultAnswer,Constraint,Abort,Error) :-
  tapi(off),
  !,
%  exec_if_verbose_on(list_rules_wo_number(RelationName,Arity,2,_ODLs)),
  debug_dl_display_numbered_solutions(Solutions),  
  debug_dl_build_user_question(UserQuestion,Solutions,PossibleAnswers,DefaultAnswer,CompleteUserQuestion),
...
%        Constraint = u_valid(RelationName,no)
       ;
        (Str == "A"
         ->
          Abort=abort
         ;
          (memberchk(u_missing(_,_),PossibleAnswers),
           ...
           ;
            (memberchk(u_wrong(_,_),PossibleAnswers),
  ...
  question_answer_constraint(Question,Solutions,Answer,Constraint).
% TAPI
debug_dl_input_ask_user(Question,_UserQuestion,_RelationName,_Arity,_Solutions,_PossibleAnswers,_DefaultAnswer,_Constraint,_Abort,_Error) :-
%   length(Solutions,NbrTuples),
%   add_to_flag(debug_tuples,NbrTuples),
  inc_flag(debug_questions),
  set_flag(debug_dl_current_question,Question).
...
debug_dl_build_subrel_cts(Predicates,Rules) -->
  {findall(subRel(DBName,Name),
           (member(Name/Arity,Predicates),
            functor(H,Name,Arity),
%             member((':-'(H,B),_NVs),Rules),
%             functor(B,BName,Arity),
            member((':-'(H,DB),_NVs),Rules),
            functor(DB,DBName,Arity),
%            atom_concat('$',BName,DBName),
            atom_concat('_',BName,DBName),
...
debug_dl_build_body_cts(Predicates,Rules) -->
  {findall(bodyRel(DBName,Tuples),
           (member(Name/Arity,Predicates),
            functor(H,Name,Arity),
%             member((':-'(H,B),_),Rules),         % p(A):-p1(A)
%             functor(B,BName,Arity),
            member((':-'(H,DB),_),Rules),         % p(A):-$p1(A)
            functor(DB,DBName,Arity),
%            atom_concat('$',BName,DBName),
            atom_concat('_',BName,DBName),
            atom_concat(Name,NumberAtom,BName),
            ...
            findall((LName,Sign,WrongName,MissingName),
                    (my_nth1_member(Lj,J,Bis),             % s(X)
                     (Lj = not(NL)
                      ->
                       Sign = neg, 
                       functor(NL,LName,LArity) 
                      ;
                       Sign = pos, 
                       functor(Lj,LName,LArity)),
                     not_builtin(LName/LArity),
%                      atomic_concat_list(['$',w,BName,J],WrongName),
%                      atomic_concat_list(['$',m,BName,J],MissingName)
                     debug_dl_intermediate_name([w,BName,J],WrongName),
                     debug_dl_intermediate_name([m,BName,J],MissingName)
...
%  {get_source_dlrules(namearity,Predicate,DLs),
  {get_des_object_dlrules(namearity,Predicate,DLs),
...
%     atomic_concat_list(['$',Name,N],NB),
     debug_dl_intermediate_name([Name,N],NB),
     Ri =.. [NB|Xs],
...
%     atomic_concat_list(['$',h,Name,N],NB),
     debug_dl_intermediate_name([h,Name,N],NB),
     H =.. [Name|Tis],
     HRi =.. [NB|Tis]
    ),
    TRules
...
%   atomic_concat_list(['$',nh,Name],NH),
   debug_dl_intermediate_name([nh,Name],NH),
   NHR =.. [NH|Xs],
   bagof(not(HRi),
    Rule^N^NB^
...
%      atomic_concat_list(['$',w,Name,I,J],FWRIJ),
      debug_dl_intermediate_name([w,Name,I,J],FWRIJ),
      S =.. [_FS|Sks],
...
%      atomic_concat_list(['$',m,Name,I,J],FMRIJ),
      debug_dl_intermediate_name([m,Name,I,J],FMRIJ),
      concat_lists([Tis,Sks],TiSks),
      MRIJ =.. [FMRIJ|TiSks],
...
%  atomic_concat_list(['$',h,Name],FHRC),
  debug_dl_intermediate_name([h,Name],FHRC),
  Ri =.. [_|Tis], 
  L =.. [FHRC|Tis],
  C1 is C+1,
  build_not_literals_5b(Rs,Ri,C1,I,NLs).
  
debug_dl_intermediate_name(Xs,Y) :-
  debug_dl_intermediate_name_aux(Xs,Ys),
%  atomic_concat_list(['$'|Ys],Y).
  atomic_concat_list(['_'|Ys],Y).
% debug_dl_intermediate_name(Xs,Y) :-
%   atomic_concat_list(Xs,Y).

debug_dl_intermediate_name_aux([],[]).
debug_dl_intermediate_name_aux([X|Xs],[Y|Ys]) :-
  atom(X),
%  atomic_concat('$',Y,X),
  atomic_concat('_',Y,X),
  !,
  debug_dl_intermediate_name_aux(Xs,Ys).
debug_dl_intermediate_name_aux([X|Xs],[X|Ys]) :-
  debug_dl_intermediate_name_aux(Xs,Ys).
...
%             my_abolish(ft/3),                % Remove all clauses of the predicate "ft"
  ...
  set_flag(debug_root_tuples,Cardinality),
  set_flag(debug_questions,0),
  set_flag(debug_tuples,0),
