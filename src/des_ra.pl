...
%   |
%   TOP IntegerExpression (RArel)        % Top-N query
%   
% RArel ::=
%   RAstmt
%   |
...
% RA DDL Statement
my_RA_DDL_stmt(assign(select(true,Relation),Schema)) -->
  my_create_view_schema(Schema),
  my_sql_blanks_star, 
  ":=",
  my_sql_blanks_star, 
  my_RA_relation(Relation).
my_RA_DDL_stmt(assign(RAst,Schema)) -->
  my_create_view_schema(Schema),
  my_sql_blanks_star, 
  ":=",
  my_sql_blanks_star, 
...
my_RA_DQL_stmt(E) -->
  push_syntax_error(['Expected valid RA DQL statement (SELECT, PROJECT, RENAME, SORT, TOP, DISTINCT, GROUP_BY or an expression involving infix operators as PRODUCT, UNION, DIFFERENCE, INTERSECT, DIVISION and outer joins)'],Old),
  my_term_RA_DQL(T),
  my_RA_DQL_r(T,E),
  {valid_RA_stmt(T,E)},
...
valid_RA_stmt(T,T) :-
  !,
  \+ atomic(T). % A RA relation is not a valid RA statement
valid_RA_stmt(_,_).
  
my_RA_DQL(E) -->
  push_syntax_error(['Expected valid RA expression (SELECT, PROJECT, RENAME, SORT, TOP, DISTINCT, GROUP_BY or an expression involving infix operators as PRODUCT, UNION, DIFFERENCE, INTERSECT, DIVISION and outer joins)'],Old),
  my_term_RA_DQL(T),
  my_RA_DQL_r(T,E),
  pop_syntax_error(Old).
...
my_factor_RA_relation(select(Cond,RA)) -->
  my_kw("SELECT"),
  push_syntax_error(['Expected condition'],Old),
  my_sql_blanks,
  my_sql_condition(Cond),
  pop_syntax_error(Old),
  my_sql_blanks_star,
...
my_factor_RA_relation(project(ProjList,RA)) -->
  my_kw("PROJECT"),
  push_syntax_error(['Expected projection list'],Old),
  my_sql_blanks,
...
my_factor_RA_relation(sort(OrderArgs,OrderSpecs,RA)) -->
  my_kw("SORT"),
  push_syntax_error(['Expected ordering criteria list'],Old),
  my_sql_blanks,
...
my_factor_RA_relation(top(N,RA)) -->
  my_kw("TOP"),
  push_syntax_error(['Expected integer expression'],Old),
  my_sql_blanks,
  % my_integer(N),
  my_top_argument(N), % WARNING: This should not read SQL expression but it does
  pop_syntax_error(Old),
  my_sql_blanks_star,
  my_b_RA_DQL(RA).
% rename
my_factor_RA_relation(rename(Schema,RA)) -->
  my_kw("RENAME"),
  push_syntax_error(['Expected relation schema'],Old),
  my_sql_blanks,
...
my_factor_RA_relation(group_by(GroupList,ProjList,Cond,RA)) -->
  my_kw("GROUP_BY"),
  push_syntax_error(['Expected group_by list'],Old1),
  my_sql_blanks,
  my_column_list_or_nil(GroupList),
  pop_syntax_error(Old1),
  push_syntax_error(['Expected projection list'],Old2),
  my_sql_blanks,
  my_projection_list(ProjList),
  pop_syntax_error(Old2),
  push_syntax_error(['Expected having condition'],Old3),
  my_sql_blanks,
...
% Bracketed RA DQL
my_b_RA_DQL(RA) -->
  push_syntax_error(['Expected opening parenthesis'],Old1),
  "(",
  pop_syntax_error(Old1),
  my_sql_blanks_star,
  my_RA_DQL(RA),
  my_sql_blanks_star,
  push_syntax_error(['Expected closing parenthesis'],Old2),
  ")",
  pop_syntax_error(Old2).
...
write_ra(group_by(GroupList,ProjList,Cond,Relation),I) :-
  ...
  write_expr_list(GroupList,0,'$des'),
  write_log(' '),
