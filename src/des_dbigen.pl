...
mutate_query(
            select(DistinctAll,Top,Offset,Exprs,[],from(Rs),where(Cs),group_by([]),having(true),order_by([],[])),
            select(DistinctAll,Top,Offset,Exprs,[],from(Rs),where(ACs),group_by([]),having(true),order_by([],[]))) :-
...
generate_DQL_type_stmt(union,View/Arity,PDG,TableSize,union(DistinctAll,(SQLstL,_),(SQLstR,_))) :-
  ...
  generate_DQL_type_stmt(select,View/Arity,PDGL,TableSize,SQLstL),
  generate_DQL_type_stmt(select,View/Arity,PDGR,TableSize,SQLstR).
generate_DQL_type_stmt(select,View/Arity,(_Nodes,Arcs),TableSize,select(DistinctAll,top(all),no_offset,Exprs,[],from(Rs),where(Cs),group_by([]),having(true),order_by([],[]))) :-
  select_random_list([distinct,all],DistinctAll),
...
enlarge_query_results(N/A,T,Query,MQuery) :-
  ...
  EQuery=select(all,top(all),no_offset,*,[],from([(AQuery,_)]),where(true),group_by([]),having(true),order_by([],[])), % AQuery must be encapsulated in a SELECT * FROM (e.g., for a union)
  CQuery=select(all,top(all),no_offset,[expr(count,_,_)],[],from([(EQuery,_)]),where(true),group_by([]),having(true),order_by([],[])),
...
get_random_seed(Seed) :-
  my_current_datetime((_Y,_M,D,H,Mi,S)),
