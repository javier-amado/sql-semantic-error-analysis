...
% CLP(R) constraint library
:- use_module(library(clpr),[]). % This allows to use both clpr and clpq, prepending either clpr: or clpq: to the constraint goal, as e.g. clpr:{X*3=1)}

% CLP(Q) constraint library
:- use_module(library(clpq),[]). % This allows to use both clpr and clpq, prepending either clpr: or clpq: to the constraint goal, as e.g. clpr:{X*3=1)}
...
%  odbc_table_column(Handle,ODBCTableName,ColName,type_name(TypeName)),
  odbc_table_column(Handle,ODBCTableName,ColName,data_type(TypeNameCode)),
  my_odbc_datatype_typename(TypeNameCode,TypeName),
%  odbc_table_column(Handle,ODBCTableName,ColName,length(TypeLength)),
%   odbc_table_column(Handle,ODBCTableName,ColName,precision(TypeLength)),
...
%%
  my_odbc_table_column_type(Handle,ODBCTableName,ColName,TypeName,Type).

my_odbc_table_column_type(Handle,ODBCTableName,ColName,IntOrFloat,Type) :-
  (IntOrFloat=integer ; IntOrFloat=float), 
  !,
  odbc_table_column(Handle,ODBCTableName,ColName,length(LENGTH)),
  Type=..[IntOrFloat,LENGTH],
  !.
my_odbc_table_column_type(Handle,ODBCTableName,ColName,decimal,Type) :-
  !,
  odbc_table_column(Handle,ODBCTableName,ColName,precision(PRECISION)),
  odbc_table_column(Handle,ODBCTableName,ColName,scale(SCALE)),
  Type=..[decimal,PRECISION,SCALE],
  !.
my_odbc_table_column_type(Handle,ODBCTableName,ColName,unknown,Type) :-
  !,
  odbc_table_column(Handle,ODBCTableName,ColName,type_name(UpperTypeName)),
  odbc_table_column(Handle,ODBCTableName,ColName,precision(PRECISION)),
  to_lowercase(UpperTypeName,TypeName),
  Type=..[TypeName,PRECISION],
  !.
my_odbc_table_column_type(Handle,ODBCTableName,ColName,TypeName,Type) :-
  odbc_table_column(Handle,ODBCTableName,ColName,precision(PRECISION)),
  Type=..[TypeName,PRECISION],
...
my_odbc_get_query_typed_arguments(Connection,SQLstr,ColNameTypes) :-  
  ...
  concat_lists(["CREATE VIEW ",TableNameStr," AS ",SQLstr],CreateStr),
         ...
         (
      	 ...
      	 ((verbose(on)
      	   ->
      	    (my_exception_message_display(_,Message)
      	    ;
...
%  stream_property(CurrentStream,position(CurrentPos)),
  read_term(Term, [variable_names(NAVariableNames),variables(Variables),term_position(CurrentPos)]),
...
%  stream_position_data(line_count,NewPos,Max).
  stream_property(CurrentStream,position(NewPos)),
  stream_position_data(line_count,NewPos,Max).
...
reset_elapsed_time :-
  ...
  retractall(query_elapsed_time(_,_,_)),
  assertz(query_elapsed_time(_,_,_)),
...
get_command_elapsed_time(Elapsed) :-
%  elapsed_time(T1),
%  get_time(T2),
%  Elapsed is integer(1000*(T2-T1)).
  get_time(Time),
  Elapsed is integer(1000*Time).
...
my_subsumes_term(X,Y) :-
  % subsumes_chk(X,Y). % linter
  subsumes_term(X,Y).
...
% Term variants: equal wrt. structure
variants(A,B) :-
  A=@=B.
...
my_create_mutable(Value,Cell) :-
  create_mutable(Value,Cell).

my_update_mutable(Value,Cell) :-
  update_mutable(Value,Cell).

my_get_mutable(Value,Cell) :-
  get_mutable(Value,Cell).
