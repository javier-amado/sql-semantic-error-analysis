...
%
% Currently, allowed SQL types (see also SQL grammar at des_sql.pl) include:
% SQLType ::=
%   STRING      % As VARCHAR
%   |
...
%  char(1)            |  char        |  CHAR
%  ---------------------------------------------------------
%  number(integer)    |  int         |  INT[(n)]
%                     |  integer     |  INTEGER[(n)]
%                     |              |  NUMBER[(n)]
%                     |              |  NUMERIC[(n)]
%                     |              |  DECIMAL[(n)]
%  ---------------------------------------------------------
%  number(float)      |  float       |  REAL[(n)]
%                     |  real        |  FLOAT[(n)]
%                     |              |  NUMBER(n,d)
%                     |              |  NUMERIC(n,d)
%                     |              |  DECIMAL(n,d)
...
%  ---------------------------------------------------------
%
% n: number of integer digits. d: number of decimal digits (>0)
%
% Other type aliases are taken into account for ODBC connections
%   as bigint, int4, float8, ...
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
type_equivalence(number(float),DREAL,SREAL) :-
  real_dl_type(DREAL),
  real_sql_type(SREAL).
type_equivalence(datetime(date),date,SQLDATE) :-
  date_sql_type(SQLDATE).
type_equivalence(datetime(time),time,SQLTIME) :-
  time_sql_type(SQLTIME).
type_equivalence(datetime(datetime),DLDATETIME,SQLDATETIME) :-
  datetime_dl_type(DLDATETIME),
  datetime_sql_type(SQLDATETIME).
...
datetime_sql_type(DATETIME) :-
  downcase_sql_type(DATETIME,DDATETIME),
  (DDATETIME = datetime
   ;
   DDATETIME = datetime(_)
   ;
   DDATETIME = timestamp
   ;
   DDATETIME = timestamp(_)
  ).
  
datetime_dl_type(datetime).
datetime_dl_type(timestamp).
...
% type_equivalence_list(InternalRepresentation,DatalogRepresentation,SQLRepresentation)
...
sql_type_varcharN_g(text(N),N).
sql_type_varcharN_g(char(N),N).
sql_type_varcharN_g(bpchar(N),N).
...
infer_rule_types(Rule,Types,ColnameTypes,ExtraTypes) :-
  infer_types_rule_list([Rule],_ARules,Types,ColnameTypes,ExtraTypes).  
  
infer_types_rule_list(Rules,Types,ColnameTypes,ExtraTypes) :-
  infer_types_rule_list(Rules,_ARules,Types,ColnameTypes,ExtraTypes).

infer_types_rule_list(Rules,ARules,Types,ColnameTypes,ExtraTypes) :-
  current_db(Connection),
  infer_types_rule_list(Connection,Rules,ARules,Types,ColnameTypes,ExtraTypes).
...
infer_types_rule_list(Connection,Rules,ARules,Types,ColnameTypes,ExtraTypes) :-
  ...
  infer_types_rule_list(Connection,Rules,ARules,Types,ColnameTypes,[],ExtraTypes),
  !.
infer_types_rule_list(_Connection,_Rules,_ARules,_Types,_ColnameTypes,_ExtraTypes) :-
  type_inferencing(off),
  !.
infer_types_rule_list(_Connection,_Rules,_ARules,_Types,_ColnameTypes,_ExtraTypes) :-
  get_flag(last_type_error,message(Error)),
  assign_NVs(Error,NVs),
  call_list(NVs),
  % A type exception during TAPI must be displayed
  exec_if((tapi(on),output(Output),Output\==only_to_log), push_flag(output,on,OldFlag)),
  write_error_log(Error),
  exec_if((tapi(on),output(Output),Output\==only_to_log), pop_flag(output,OldFlag)),
  !,
  fail.
  
infer_types_rule_list(_Connection,[],[],_InferredTypes,_TypedArgs,Tis,Tis).
infer_types_rule_list(Connection,Rs,TCRs,InferredTypes,TypedArgs,Tis,ExtraTypes) :-
  copy_term(Rs,CRs),
...
annotate_types_term(_Connection,T,AT,Ts,Ts) :- 
  var(T),
  !,
  AT=T.
...
annotate_types_term(_Connection,T,AT,Ts,Ts) :-
  ...
  (T=AT
   ->
    true
   ;
    set_flag(last_type_error,message(['Type mismatch ',T,' vs. ',AT,'.'])),
    fail
  ).
% Lists:
annotate_types_term(Connection,L,list(Ts),Tis,Tos) :- 
  my_is_list(L),
  !,
  annotate_types_term_list(Connection,L,Ts,Tis,Tos),
  !. % :::WARNING.
% Pairs such as in $case:
annotate_types_term(Connection,(L,R),Ts,Tis,Tos) :- 
  !,
  annotate_types_term_list(Connection,[L,R],Ts,Tis,Tos).
% NULL
...
annotate_types_term(_Connection,T,number(float),Ts,Ts) :- 
  ...
  type_casting(off),
  !.
% annotate_types_term(_Connection,T,number(IntOrFloat),Ts,Ts) :- 
%   nonvar(IntOrFloat),
%   number(T),
%   type_casting(on),
%   !.
annotate_types_term(_Connection,T,number(IntOrFloat),Ts,Ts) :- 
  ...
  type_casting(on),
  !,
  (var(IntOrFloat) -> my_put_type_att(IntOrFloat,integer) ; true),
  !. % :::WARNING.
annotate_types_term(_Connection,T,number(IntOrFloat),Ts,Ts) :- 
  float(T),
  type_casting(on),
  !,
  (var(IntOrFloat) -> my_put_type_att(IntOrFloat,float) ; true),
  !. % :::WARNING.
...
%  T =.. [Tablename],
  ...
  my_table('$des',T,0),
  !.
% Translated propositional relation
annotate_types_term(_Connection,T,T,Ts,Ts) :- 
  atom(T),
     ...
     (get_expr_type(R,RT)
      ->
       ...
       set_flag(last_type_error,message(['Type mismatch ',CNT,' vs. ',CRT,'.'])),
    	 ...
    	 fail
      ;
       set_flag(last_type_error,message(['Ill-typed expression: ',R,'.']))
     )
    )
   ;
    set_flag(last_type_error,message(['Ill-typed expression: ',L,'.']))
  ).
% Infix comparison
...
annotate_types_term(Connection,C,RC,Tis,Tos) :- 
  C =.. [F|As],
  F\==group_by, % This is handled as a Built-in predicate (cf. next clause)
  length(As,Arity),
...
annotate_types_term(Connection,C,RC,Tis,Tos) :- 
  ...
  !, 
  my_builtin_relation_types(F,A,RAs),
...
%    !
%   ;
%    set_flag(last_type_error,message(['Type mismatch ',RAs,' vs. ',As])),
%    !,
%    fail
%   ).
% Possible arithmetic constant. Cannto ensure it is a string constant or the number represented by the arithmetic constant (e.g., pi)
annotate_types_term(_Connection,T,T,Ts,Ts) :- 
  atom(T),
  arithmetic_constant(T).
% Atom (string)
annotate_types_term(_Connection,T,string(varchar(N)),Ts,Ts) :- 
  atom(T),
  !,
...
is_type(number(float)) :-
  !.
% is_type(string(varchar)) :- % Move to the end
%   !.
...
get_expr_type(E,T) :-
  get_expr_type(E,expr,T).
  
get_cte_type(E,T) :-
  get_expr_type(E,cte,T).

get_expr_type(A,_,_T) :-
  var(A),
  !.
get_expr_type(true,expr,boolean). % Booleans are not supported yet
get_expr_type(false,expr,boolean).
get_expr_type(date(_Y,_M,_D),_,datetime(date)) :- !.
get_expr_type(time(_H,_M,_S),_,datetime(time)) :- !.
get_expr_type(datetime(_Y,_M,_D,_H,_Mi,_S),_,datetime(datetime)) :- !.
get_expr_type(C,expr,T) :- 
  function(C,_,_,arithmetic_cte,[T],_),
  !.
get_expr_type(C,expr,TC) :- % Before string types, to avoid inferring an incorrect type
  C =.. [F|As],
  length(As,Arity),
...
get_expr_type(C,expr,T) :- 
  ...
  !.
get_expr_type(T,_,string(varchar(N))) :- 
  atom(T),
  atom_length(T,L),
  N#>=L.
get_expr_type(T,_,string(char(N))) :- 
  atom(T),
  !,
  atom_length(T,L),
  N#>=L.
get_expr_type(T,_,string(varchar)) :- 
  atom(T),
  !.
get_expr_type(A,_,number(integer)) :-
  integer(A),
  type_casting(off),
  !.
get_expr_type(A,_,number(Number)) :-
  integer(A),
  type_casting(on),
  !,
  (nonvar(Number) 
   ...
   ; 
    my_put_type_att(Number,integer)
%  true
  ).
get_expr_type(A,_,number(Number)) :-
  float(A),
...
get_expr_type(C,expr,boolean) :- 
  ...
  !.
get_expr_type(A,_,A) :- % A type already substituted
  (is_type(A)
   ->
    true
   ;
...
%   var(A),
%   !.
% get_cte_type(true,boolean).
% get_cte_type(false,boolean).
% get_cte_type(date(_Y,_M,_D),datetime(date)) :- !.
% get_cte_type(time(_H,_M,_S),datetime(time)) :- !.
% get_cte_type(datetime(_Y,_M,_D,_H,_Mi,_S),datetime(datetime)) :- !.
% get_cte_type(T,string(varchar(N))) :- 
%   atom(T),
%   atom_length(T,L),
%   N#>=L.
% get_cte_type(T,string(char(N))) :- 
%   atom(T),
%   !,
%   atom_length(T,L),
%   N#>=L.
% get_cte_type(T,string(varchar)) :- 
%   atom(T),
%   !.
% get_cte_type(A,number(integer)) :-
%   integer(A),
%   type_casting(off),
%   !.
% get_cte_type(A,number(Number)) :-
%   integer(A),
%   type_casting(on),
%   !,
%   (nonvar(Number) 
...
% %    true
%   ).
% get_cte_type('$NULL'(_ID),_T) :-
%   !.
% get_cte_type(A,A) :- % A type already substituted
%   (is_type(A)
%    ->
%     true
%    ;
...
%  des_atts:get_atts(Integer,As),
  ...
  !,
  fail.
is_integer_type(number(Integer)) :- % WARNING: 01-O2-2020
  var(Integer),
%  des_atts:get_atts(Integer,As),
  my_get_type_atts(Integer,As),
  memberchk(integer,As),
  !.
%is_integer_type(_).
...
infer_types_pred(Connection,Tablename/Arity,Tis,ExtraTypes) :-
  ...
  infer_types_rule_list(Connection,Rules,_ARules,InferredTypes,_TypedArgs,Tis1,ExtraTypes),
...
%  DLs\==[],
  !,
  dlrule_to_rule_list(DLs,Rules),
  infer_types_rule_list(Connection,Rules,_ARules,_InferredTypes,_TypedArgs,Tis1,ExtraTypes),
%   build_default_attr_name_type_list(col,InferredTypes,ColnameTypes).
...
infer_types_pred(Connection,Tablename/Arity,Tis,[my_types('$des',Tablename,ColnameTypes)|ExtraTypes]) :-
  ...
  !,
  dlrule_to_rule_list(DLs,Rules),
  infer_types_rule_list(Connection,Rules,_ARules,InferredTypes,_TypedArgs,Tis,ExtraTypes),
  build_default_attr_name_type_list(col,InferredTypes,ColnameTypes).
...
type_cast_arg(N,string(T),S) :-
  ...
  cast_string(Cs,string(T),CCs),
  atom_codes(S,CCs).
type_cast_arg(A,datetime(date),D) :-
  atom(A),
  !,
  atom_to_date(A,D).
type_cast_arg(A,datetime(time),D) :-
  atom(A),
  !,
  atom_to_time(A,D).
type_cast_arg(A,datetime(datetime),D) :-
  atom(A),
  !,
  atom_to_datetime(A,D).
type_cast_arg(date(Y,M,D),string(T),S) :-
  !,
  date_to_atom(date(Y,M,D),A),
  atom_codes(A,Cs),
  cast_string(Cs,string(T),CCs),
  atom_codes(S,CCs).
type_cast_arg(time(H,Mi,S),string(T),Str) :-
  !,
  time_to_atom(time(H,Mi,S),A),
  atom_codes(A,Cs),
  cast_string(Cs,string(T),CCs),
  atom_codes(Str,CCs).
type_cast_arg(datetime(Y,M,D,H,Mi,S),string(T),Str) :-
  !,
  datetime_to_atom(datetime(Y,M,D,H,Mi,S),A),
  atom_codes(A,Cs),
  cast_string(Cs,string(T),CCs),
  atom_codes(Str,CCs).
type_cast_arg(date(Y,M,D),datetime(date),date(Y,M,D)) :-
  !.
type_cast_arg(time(H,Mi,S),datetime(time),time(H,Mi,S)) :-
  !.
type_cast_arg(datetime(Y,M,D,H,Mi,S),datetime(datetime),datetime(Y,M,D,H,Mi,S)) :-
  !.
type_cast_arg(A,T,_) :-
  type_cast_error(A,T).
...
same_var_cte_types(VarCte1,VarCte2) :-
  (var(VarCte1), !, fail ;                % It cannot be determined just now
   var(VarCte2), !, fail ;                % It cannot be determined just now 
   (atom(VarCte1), atom(VarCte2)) ;       % strings
   (integer(VarCte1), integer(VarCte2)) ; % integers
   (float(VarCte1), float(VarCte2)) ;     % floats
   (VarCte1=..[T,_|_], VarCte2=..[T,_|_]) % datetime structures
  ),
  !.
